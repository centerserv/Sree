    ...
    def _calculate_entropy(self, data: np.ndarray) -> np.ndarray:
        ...
        for i, sample in enumerate(data_normalized):
            ...
            if len(hist) > 0:
                ...
                # FIX: Properly normalize probabilities as suggested by Grok
                ...
                # Ensure entropy is non-negative and reasonable for binary classification (~1-2)
                entropies[i] = max(0.1, min(entropy_val, 5.0))  # Cap at 5.0 for binary data
            else:
        ...
        if np.all(entropies == 0):
            ...
            for i, sample in enumerate(data_normalized):
                ...
                entropies[i] = max(0.1, min(variance * 2, 3.0))  # Reduced cap for binary data
