...
class PermanenceValidator(Validator):
    ...
    def validate(self, data: np.ndarray, labels: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        # FIX: Dynamic block creation as suggested by Grok
        # Call _finalize_block repeatedly until we reach target block count
        target_blocks = 4  # Target >4 blocks as suggested by Grok
        max_iterations = 10  # Prevent infinite loops
        
        iteration = 0
        while len(self._ledger) < target_blocks and len(self._current_block) > 0 and iteration < max_iterations:
            # Check if we have enough records to create a block
            if len(self._current_block) >= self._block_size // 3:  # More aggressive block creation
                self._finalize_block()
            elif len(self._current_block) >= self._block_size // 2 and len(self._ledger) < 2:
                # Ensure we get at least 2 blocks
                self._finalize_block()
            elif len(self._current_block) > 0 and len(self._ledger) == 0:
        ...
        logger.info(f"Permanence processed {len(validation_records)} records, "
                   ...
                   f"total blocks: {len(self._ledger)} (target: {target_blocks})")
