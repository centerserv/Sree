...
class SREEDashboard:
    ...
    def __init__(self):
        ...
        # Initialize credit data analyzer
        self.credit_analyzer = CreditDataAnalyzer()
        ...
        if uploaded_file is not None:
            try:
                ...
                if target_column and feature_columns:
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        run_data_analysis = st.checkbox("Run Data Quality Analysis", value=True)
                    with col2:
                        run_sree_analysis = st.checkbox("Run SREE Analysis", value=True)
                    
                    # Run analysis
                    if st.button("🚀 Run Analysis", type="primary"):
                        if run_data_analysis:
                            with st.spinner("Running data quality analysis..."):
                                analysis_results = self.credit_analyzer.analyze_dataset(X, y, feature_columns)
                                st.session_state.data_analysis_results = analysis_results
                                self.display_data_analysis_results(analysis_results, df)
                        
                        if run_sree_analysis:
    ...
    def display_data_analysis_results(self, analysis_results: dict, df: pd.DataFrame):
        """Display comprehensive data analysis results."""
        st.header("🔍 Data Quality Analysis Results")
        
        # Create tabs for different analysis sections
        tab1, tab2, tab3, tab4, tab5 = st.tabs([
            "📊 Overview", "🚨 Issues", "💡 Suggestions", "📈 Visualizations", "📋 Report"
        ])
        
        with tab1:
            self._display_analysis_overview(analysis_results)
        
        with tab2:
            self._display_data_issues(analysis_results)
        
        with tab3:
            self._display_cleaning_suggestions(analysis_results)
        
        with tab4:
            self._display_analysis_visualizations(df, analysis_results)
        
        with tab5:
            self._display_analysis_report(analysis_results)
    
    def _display_analysis_overview(self, analysis_results: dict):
        ...
        with col3:
            good_credit = info['class_distribution'].get(0, 0)
            bad_credit = info['class_distribution'].get(1, 0)
            st.metric("Good Credit", good_credit)
        with col4:
            st.metric("Bad Credit", bad_credit)
        ...
        correlations = analysis_results['correlation_analysis']['target_correlations']
        ...
        def color_correlation(val):
            if abs(val) < 0.1:
                return 'background-color: #ffcccc'  # Light red for low correlation
            elif abs(val) < 0.3:
                return 'background-color: #ffffcc'  # Light yellow for medium correlation
            else:
                return 'background-color: #ccffcc'  # Light green for high correlation
        
        st.dataframe(corr_df.style.applymap(color_correlation, subset=['Correlation']))
    
    def _display_data_issues(self, analysis_results: dict):
        """Display data quality issues."""
        issues = analysis_results['data_quality_issues']
        outliers = analysis_results['outlier_analysis']
        problematic = analysis_results['problematic_individuals']
        ...
        domain_outliers = outliers['domain_specific']
        total_outliers = sum(len(data['indices']) for data in domain_outliers.values())
        
        if total_outliers > 0:
            st.warning(f"⚠️ Found {total_outliers} problematic individuals")
            
            for outlier_type, data in domain_outliers.items():
                if data['count'] > 0:
                    with st.expander(f"{outlier_type.replace('_', ' ').title()} ({data['count']} individuals)"):
                        st.write(f"**Description:** {data['description']}")
                        st.write(f"**Indices:** {data['indices'][:10]}{'...' if len(data['indices']) > 10 else ''}")
        else:
            st.success("✅ No outliers detected!")
        
        # Problematic individuals
        st.subheader("⚠️ Problematic Individuals")
        total_problematic = sum(len(data['indices']) for data in problematic.values())
        
        if total_problematic > 0:
            st.warning(f"⚠️ Found {total_problematic} individuals with issues")
            
            for problem_type, data in problematic.items():
                if data['count'] > 0:
                    with st.expander(f"{problem_type.replace('_', ' ').title()} ({data['count']} individuals)"):
                        st.write(f"**Description:** {data['description']}")
                        st.write(f"**Indices:** {data['indices'][:10]}{'...' if len(data['indices']) > 10 else ''}")
        else:
            st.success("✅ No problematic individuals found!")
    
    def _display_cleaning_suggestions(self, analysis_results: dict):
        """Display cleaning suggestions."""
        suggestions = analysis_results['cleaning_suggestions']
        ...
        if suggestions:
            ...
            if high_priority:
                ...
                for i, suggestion in enumerate(high_priority, 1):
                    with st.expander(f"{i}. {suggestion['type'].replace('_', ' ').title()}"):
            ...
            if low_priority:
                st.info("ℹ️ Low Priority Suggestions:")
                for i, suggestion in enumerate(low_priority, 1):
                    with st.expander(f"{i}. {suggestion['type'].replace('_', ' ').title()}"):
                        st.write(f"**Reason:** {suggestion['reason']}")
                        st.write(f"**Impact:** {suggestion['impact']}")
        else:
            st.success("✅ No cleaning suggestions needed!")
    
    def _display_analysis_visualizations(self, df: pd.DataFrame, analysis_results: dict):
        ...
        try:
            ...
            correlations = analysis_results['correlation_analysis']['target_correlations']
            importance_df = pd.DataFrame(list(correlations.items()), columns=['Feature', 'Correlation'])
            importance_df['Abs_Correlation'] = importance_df['Correlation'].abs()
            importance_df = importance_df.sort_values('Abs_Correlation', ascending=True)
            ...
            fig = px.bar(
                ...
                y='Feature',
                orientation='h',
                title='Feature Importance (Absolute Correlation with Target)',
            ...
            fig = px.pie(
                ...
                names=['Good Credit' if i == 0 else 'Bad Credit' for i in target_counts.index],
    ...
    def _display_analysis_report(self, analysis_results: dict):
        """Display comprehensive analysis report."""
        st.subheader("📋 Complete Analysis Report")
        
        # Generate report
        report = self.credit_analyzer.generate_report(analysis_results)
        
        # Display report
        st.text_area("Analysis Report", report, height=600)
        
        # Download button
        st.download_button(
            label="📥 Download Report",
            data=report,
            file_name=f"credit_analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
            mime="text/plain"
        )
